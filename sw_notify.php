...
add_action( 'admin_menu', 'sw_admin_notify' );
...
function sw_notify() {
    ...
    foreach ( $resultpm as $rowpm ) {
        ...
        foreach ( $result1 as $row1 ) {
            $email_array[ $row1->user_email ] .= "{$row1->region} has a high risk of <a href='http://54.206.122.93/wordpress/index.php/pm-management/'> Powdery Mildew </a> on {$rowpm->forecast_day}, {$rowpm->forecast_date}.<br/>";
        }
    }
    ...
    foreach ( $resultgm as $rowgm ) {
        ...
        foreach ( $result3 as $row3 ) {
            $email_array[ $row3->user_email ] .= "{$row3->region} has a high risk of <a href='http://54.206.122.93/wordpress/index.php/gm-management/'> Grey Mould </a> on {$rowgm->forecast_day}, {$rowgm->forecast_date}.<br/>";
        }
    }
